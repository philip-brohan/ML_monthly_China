...
# Fit to the full field for SST, and specified points ('stations') for othr variables.
...
ict = cList_to_tensor(qd, lm_ERA5.data.mask, extrapolate=True)

# Use bridson to provide station locations
lat_range = (
    lm_ERA5.coords("latitude")[0].points.min(),
    lm_ERA5.coords("latitude")[0].points.max(),
)
lon_range = (
    lm_ERA5.coords("longitude")[0].points.min(),
...
lm_stations = lm_ERA5.copy()
lm_stations.data.data.fill(0.0)
lm_stations.data.mask.fill(False)
for idx in range(len(slx)):
    lm_stations.data.data[slx[idx],sly[idx]]=1
    lm_stations.data.mask[slx[idx],sly[idx]]=True
...
target = tf.constant(tf.reshape(ict, [1, 200, 320, 4]))
    ...
    if args.SST:
        result = result + tf.reduce_mean(
            tf.keras.metrics.mean_squared_error(
                ...
                tf.boolean_mask(
                    target[:, :, :, 1], np.invert(lm_ERA5.data.mask), axis=1
                ),
            )
        )
    if args.TMP2m:
...
varx = unnormalise(varx, "mean_sea_level_pressure") / 100
...
vary = unnormalise(vary, "mean_sea_level_pressure") / 100
...
vary = unnormalise(vary, "cbrt_precipitation") * 1000
